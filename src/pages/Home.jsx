...
import { useNavigate } from 'react-router-dom';
  ...
  const Home = () => {
    const navigate=useNavigate()
      ...
      const [updatedTaskDetails, setUpdatedTaskDetails] = useState({ title: "", description: "", status: "", progress: 0 });
      ...
      const handleUpdate = async (e, id) => {
        e.preventDefault();
        //needed a fetch single task request here
        if (updatedTaskDetails.title && updatedTaskDetails.description && updatedTaskDetails.status && updatedTaskDetails.progress) {
          const reqBody = updatedTaskDetails;
          const token = sessionStorage.getItem("token");
          if (token) {
            const reqHeader = {
              "Authorization": `Bearer ${token}`
            ...
            try {
              const result = await updateTaskAPI(id, reqBody, reqHeader);
              if (result.status === 200) {
    ...
    return (
        <div className="h-[100vh]">
            ...
            {activeModal === 'update' && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="relative w-full max-w-md max-h-full bg-white rounded-lg shadow">
                        <form onSubmit={handleUpdate(task?._id)} className="p-4">
                            ...
                            <div className="grid gap-4 mb-4">
                                ...
                                <div>
                                    <label htmlFor="status" className="block mb-2 text-sm font-medium"> Status </label>
                                    <select onChange={e => setUpdatedTaskDetails({ ...updatedTaskDetails, status: e.target.value })}
                                        id="status" className="bg-gray-50 border border-gray-300 rounded-lg w-full p-2" >
                                        <option value="pending">Pending</option>
                                ...
                                <div>
                                    <label htmlFor="progress" className="block mb-2 text-sm font-medium"> Progress </label>
                                    <input onChange={e => setUpdatedTaskDetails({ ...updatedTaskDetails, progress: e.target.value })}
                                        type="number" id="progress" className="bg-gray-50 border border-gray-300 rounded-lg w-full p-2" placeholder="Enter task progress (0-100)" />
                                </div>
                            </div>
                            <button
